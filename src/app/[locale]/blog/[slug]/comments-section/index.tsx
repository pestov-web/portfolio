...
export function CommentsSection({ postId, comments, locale }: CommentsSectionProps) {
    ...
    return (
        <section>
                        ...
                        <div key={comment.id} className={commentsSectionClassNames.item}>
                            ...
                            {session && (session.user.id === comment.user.id || session.user.role === 'ADMIN') ? (
                                <form
                                    ...
                                    className={commentsSectionClassNames.deleteForm}
                                >
                                    <button
                                        type='submit'
                                        title={t('delete')}
                                        aria-label={t('delete')}
                                        className={commentsSectionClassNames.deleteButton}
                                    >
                                        ✕
                                    </button>
                                </form>
            ...
            {session ? (
                <CommentForm postId={postId} locale={locale} placeholder={t('placeholder')} submitLabel={t('submit')} />
            ) : (
