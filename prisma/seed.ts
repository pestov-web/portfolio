  ...
  const posts = [
    {
      ...
      slug: "why-typescript",
      excerpt: "Личный взгляд на то, почему строгая типизация экономит часы дебага и делает код самодокументируемым.",
      content: doc(
...
async function BlogPage() {
  ...
  for (const post of posts) {
    const existingPost = await db.post.findUnique({ where: { slug: post.slug } });
    ...
    console.log(`  ✓ Пост: "${post.title}" (${post.published ? (post.restricted ? "закрытый" : "опубликован") : "черновик"})`);
  }
  ...
  const projects = [
    {
      title: "Portfolio",
      slug: "portfolio",
      description: "Этот сайт — персональное портфолио и блог, построенный на Next.js с App Router, Prisma и Better Auth.",
      content: doc(
    ...
    },
    {
      ...
      slug: "deploy-cli",
      description: "Node.js CLI для автоматизации деплоя в Docker Swarm с поддержкой rollback и уведомлениями в Telegram.",
      content: doc(
        heading(2, "О проекте"),
        paragraph(
  ...
  // ─── Комментарии ──────────────────────────────────────────────────────────
  const publicPost = await db.post.findUnique({ where: { slug: "why-typescript" } });

  if (publicPost) {
    const existingComments = await db.comment.count({ where: { postId: publicPost.id } });
    if (existingComments === 0) {
      await db.comment.createMany({
        data: [
          ...
          {
            id: randomUUID(),
            content: "Согласен насчёт рефакторинга. Последний проект переписывал с JS на TS — TS нашёл 40+ скрытых багов.",
            postId: publicPost.id,
